...
use super::{ast::*, expand::*};

fn analyze_lexicals(iform: &P<IForm>) -> (HashSet<P<LVar>>, HashSet<P<LVar>>) {
    let mut referenced = HashSet::new();
    let mut assigned = HashSet::new();

    fn pass(iform: &IForm, referenced: &mut HashSet<P<LVar>>, assigned: &mut HashSet<P<LVar>>) {
        match &iform.term {
            ITerm::Const(_) | ITerm::GRef(_) | ITerm::PrimRef(_) => (),
            ITerm::If(test, cons, alt) => {
                pass(test, referenced, assigned);
                pass(cons, referenced, assigned);
                pass(alt, referenced, assigned);
            }
            ITerm::LRef(var) => {
            ...
            ITerm::LSet(var, val) => {
                ...
                pass(val, referenced, assigned);
            }

            ITerm::Define(_, val) | ITerm::GSet(_, val) => pass(val, referenced, assigned),
            ITerm::Seq(seq) => seq.iter().for_each(|x| pass(x, referenced, assigned)),
            ITerm::Let(l) => {
                for init in l.initializers.iter() {
                    pass(init, referenced, assigned);
                }

                pass(&l.body, referenced, assigned);
            }

            ITerm::Fix(fix) => {
                for proc in fix.procedures.iter() {
                    for case in proc.cases.iter() {
                        pass(&case.body, referenced, assigned);
                    }
                }

                pass(&fix.body, referenced, assigned);
            }
            ...
            ITerm::App(proc, args) => {
                pass(proc, referenced, assigned);
                args.iter().for_each(|a| pass(a, referenced, assigned));
            }
    ...
    pass(&**iform, &mut referenced, &mut assigned);
    (referenced, assigned)
}
...
pub fn pass_fix_letrec(x: P<IForm>) -> P<IForm> {
    ...
    fn post_order(x: &P<IForm>, pass: &mut FixPass) -> P<IForm> {
        match &x.term {
            ...
            ITerm::LSet(var, val) => {
                if pass.referenced.contains(var) {
                    return P(IForm {
                        span: x.span,
