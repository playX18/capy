...
use crate::ast::*;
use crate::number::NumberParseError;
...
impl Error {
    pub fn build_reports<'a>(&'a self, result: &mut Vec<ariadne::Report<'a, Span>>) {
        match self {
    ...
    pub fn multiple(errors: Vec<Box<Error>>) -> Self {
        Error::Multiple(errors)
    }

    pub fn single(error: Box<Error>) -> Self {
